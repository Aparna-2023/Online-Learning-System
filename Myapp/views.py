...
def dashboard(request):
    try:
        quantity = CartItem.objects.filter(user=request.user).first()
    ...
    return render(request,"dashboard.html",{'data':obj,'python':obj1,'js':obj2,'php':php,'offer':offer,'quantity':quantity})
def logout1(request):
    logout(request)
    return redirect("/")
@login_required(login_url="/")
...
def add_to_cart(request, id):
    ...
    if request.user.is_authenticated:
        cart_item, created = CartItem.objects.get_or_create(user=request.user,item=item,amount=item.price)
        if not created:
            cart_item.quantity += 1
            cart_item.amount=item.price*cart_item.quantity
            cart_item.save()

        return redirect('/dashboard',{'quantity':quantity})  
    else:
        # If the user is not authenticated, redirect to the login page
        return redirect('login')
def cartitem(request):
    course=CartItem.objects.filter(user=request.user).first()
...
def add_to_cart1(request, id):
    ...
    if request.user.is_authenticated:
        ...
        return redirect('/cartitem',{'quantity':quantity})  
    else:
        # If the user is not authenticated, redirect to the login page
        return redirect('login')
